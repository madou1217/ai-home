...
async function runProxyCommand(args, deps) {
  const action = String(args[1] || '').trim();
  const showProxyUsage = deps.showProxyUsage;
  const proxyDaemon = deps.proxyDaemon;
  ...
  if (action === 'status') {
    ...
    const auto = proxyDaemon.autostartStatus();
    if (auto.supported) {
  ...
  if (action === 'start' || !action || action.startsWith('-')) {
    try {
      ...
      const result = await proxyDaemon.start(serveArgs);
  ...
  if (action === 'restart') {
    const stopped = proxyDaemon.stop();
    if (stopped.stopped) {
      console.log(`\x1b[90m[aih]\x1b[0m proxy stopped for restart (pid=${stopped.pid})`);
    }
    try {
      const result = await proxyDaemon.start(args.slice(2));
      if (result.alreadyRunning) {
        console.log(`\x1b[90m[aih]\x1b[0m proxy already running (pid=${result.pid})`);
      } else if (result.started) {
