...
function parseServerServeArgs(rawArgs) {
  ...
  for (let i = 0; i < tokens.length; i += 1) {
    ...
    if (arg === '--backend') {
      const val = String(tokens[i + 1] || '').trim().toLowerCase();
      if (!['openai-upstream', 'codex-adapter'].includes(val)) throw new Error('Invalid --backend value');
      out.backend = val;
      i += 1;
      continue;
    }
  ...
  if (!['openai-upstream', 'codex-adapter'].includes(out.backend)) out.backend = 'codex-adapter';
