...
function resolveEnvClientKey() {
  const envCandidates = [
    ['AIH_PROXY_API_KEY', process.env.AIH_PROXY_API_KEY, '--api-key'],
    ['AIH_PROXY_CLIENT_KEY', process.env.AIH_PROXY_CLIENT_KEY, '--client-key']
  ];
...
function parseServerServeArgs(rawArgs) {
  ...
  const out = {
    ...
    strategy: String(process.env.AIH_PROXY_STRATEGY || 'random').trim().toLowerCase(),
    ...
    backend: String(process.env.AIH_PROXY_BACKEND || 'openai-upstream').trim().toLowerCase(),
    ...
    sessionAffinityTtlMs: Number(process.env.AIH_PROXY_SESSION_AFFINITY_TTL_MS || 1800000),
    sessionAffinityMaxEntries: Number(process.env.AIH_PROXY_SESSION_AFFINITY_MAX || 10000)
  };
  const envPort = String(process.env.AIH_PROXY_PORT || '').trim();
  if (envPort) {
    out.port = parseStrictIntOption(envPort, '--port', 1, 65535);
    out.portSource = 'env:AIH_PROXY_PORT';
  }
  const tokens = Array.isArray(rawArgs) ? rawArgs.slice() : [];
  for (let i = 0; i < tokens.length; i += 1) {
    const arg = String(tokens[i] || '').trim();
  ...
  if (!['codex-local', 'openai-upstream'].includes(out.backend)) out.backend = 'openai-upstream';
