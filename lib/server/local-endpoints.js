...
async function handleLocalChatCompletions(ctx) {
  ...
  const availableCount = pool.filter((a) => Date.now() >= (a.cooldownUntil || 0)).length;
  ...
  if (detail === 'no_available_account') {
    writeJson(res, 503, {
      ok: false,
      ...
      cooldownAccounts: Math.max(0, pool.length - availableCount),
      hint: `Run \`aih ${provider} add\` (or \`aih ${provider} unlock <id>\`) and then \`aih server restart\`.`
    });
    return;
  }
