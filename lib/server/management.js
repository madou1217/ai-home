...
function buildManagementStatusPayload(state, options) {
  ...
  const stickyCodex = state.sessionAffinity && state.sessionAffinity.codex instanceof Map
    ? state.sessionAffinity.codex.size
    : 0;
  const stickyGemini = state.sessionAffinity && state.sessionAffinity.gemini instanceof Map
    ? state.sessionAffinity.gemini.size
    : 0;
  return {
    ok: true,
    ...
    sessionAffinity: {
      codex: stickyCodex,
      gemini: stickyGemini,
      total: stickyCodex + stickyGemini
    },
    queue: {
...
function applyReloadState(state, runtimeAccounts) {
  ...
  if (state.sessionAffinity && typeof state.sessionAffinity === 'object') {
    state.sessionAffinity.codex = new Map();
    state.sessionAffinity.gemini = new Map();
  }
  state.modelsCache = {
    updatedAt: 0,
    ids: [],
