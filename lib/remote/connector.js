...
const NON_RETRYABLE_RECONNECT_CODES = new Set([
  'AUTH_FAILED',
  'VERSION_MISMATCH',
  'BAD_AUTH',
  'BAD_HELLO'
...
class RemoteConnector extends EventEmitter {
  constructor(options) {
    ...
    this.trackedRemoteSessions = new Set();
  ...
  async openSession(target) {
    ...
    const remoteSessionId = decodeToken(parts[1] || '');
    if (remoteSessionId) {
      this.trackedRemoteSessions.add(remoteSessionId);
    }
    return {
      remoteSessionId,
  ...
  async resumeSession(remoteSessionId) {
    ...
    const parts = splitParts(line);
    const resumedId = decodeToken(parts[1] || sid);
    if (resumedId) {
      this.trackedRemoteSessions.add(resumedId);
    }
    return {
      remoteSessionId: resumedId,
      resumed: true
    };
  }
  ...
  async _connectAndHandshake(fromReconnect) {
    ...
    try {
      ...
      await this._reattachTrackedSessions();
      this.lastPongAt = Date.now();
  ...
  _onSocketClose(hadError) {
    ...
    this._scheduleReconnect(hadError ? 'socket_error' : 'socket_closed', null);
  }

  _scheduleReconnect(reason, details) {
    ...
    this.emit('reconnecting', {
      ...
      attempt: this.reconnectAttempts,
      delayMs,
      reasonCode: details && details.reasonCode ? details.reasonCode : undefined,
      reasonMessage: details && details.reasonMessage ? details.reasonMessage : undefined,
      retryable: details ? details.retryable : true
    });
    ...
    this.reconnectTimer = setTimeout(async () => {
      ...
      try {
        ...
        const reconnectFailure = this._buildReconnectFailure(err);
        this.lastReconnectError = reconnectFailure;

        this.emit('reconnect_failed', {
          reason: reconnectFailure.reasonMessage,
          reasonCode: reconnectFailure.reasonCode,
          retryable: reconnectFailure.retryable,
          attempt: this.reconnectAttempts
        });

        if (!reconnectFailure.retryable) {
          this._transition(REMOTE_STATES.CLOSED, { reason: 'non_retryable_failure', ...reconnectFailure });
          this.emit('closed', { reason: 'non_retryable_failure', ...reconnectFailure });
          return;
        }

        this._scheduleReconnect('reconnect_failed', reconnectFailure);
      }
    }, delayMs);
  }

  async _reattachTrackedSessions() {
    if (!this.trackedRemoteSessions.size) {
      return;
    }

    for (const remoteSessionId of [...this.trackedRemoteSessions]) {
      try {
        const line = await this._sendCommand(
          `RESUME_SESSION ${encodeToken(remoteSessionId)}`,
          ['SESSION_RESUMED '],
          this.timeouts.responseMs
        );
        const parts = splitParts(line);
        const resumedId = decodeToken(parts[1] || remoteSessionId);
        if (resumedId && resumedId !== remoteSessionId) {
          this.trackedRemoteSessions.delete(remoteSessionId);
          this.trackedRemoteSessions.add(resumedId);
        }
        this.emit('session_reattached', {
          remoteSessionId: resumedId || remoteSessionId
        });
      } catch (err) {
        if (err && err.code === 'INVALID_SESSION') {
          this.trackedRemoteSessions.delete(remoteSessionId);
          this.emit('session_reattach_failed', {
            remoteSessionId,
        ...
        const reasonCode = err && err.code ? err.code : 'SESSION_REATTACH_FAILED';
        const reasonMessage = err && err.message ? err.message : 'failed to reattach remote session';
        throw makeError(reasonCode, reasonMessage);
      }
    }
  }
  ...
  _connectionSnapshot() {
    return {
      ...
      controlSessionId: this.controlSessionId,
      resumed: this.lastAuthResumed,
      trackedRemoteSessionCount: this.trackedRemoteSessions.size,
  ...
  _buildReconnectFailure(err) {
    const reasonCode = err && err.code ? String(err.code) : 'RECONNECT_FAILED';
    const reasonMessage = err && err.message ? String(err.message) : 'reconnect failed';
    const retryable = !NON_RETRYABLE_RECONNECT_CODES.has(reasonCode);
    return {
      reasonCode,
      reasonMessage,
      retryable
