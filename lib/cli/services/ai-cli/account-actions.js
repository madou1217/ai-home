...
function runAiCliAccountAction(cliName, accountAction, actionArgs, deps = {}) {
  ...
  if (accountAction !== 'import') {
    ...
    log(`\x1b[90mUsage:\x1b[0m aih ${cliName} account import [sourceDir] [--dry-run]`);
    exit(1);
    return;
  }
  ...
  Promise.resolve(importer(actionArgs, {
    ...
    error(`\x1b[31m[aih] ${cliName} account import failed: ${e.message}\x1b[0m`);
    exit(1);
  });
}
