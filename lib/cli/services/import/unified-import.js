...
function createUnifiedImportService(options = {}) {
  ...
  async function importFixedProviderSource(sourceDir, parsed, importParallel, onProgress) {
  ...
  async function importDirectorySource(sourcePath, parsed, importParallel, onProgress) {
    if (parsed.provider) {
      ...
      const result = await importFixedProviderSource(sourceDir, parsed, importParallel, onProgress);
    return {
      type: 'directory',
      source: sourcePath,
  ...
  async function importZipSource(zipPath, parsed, importParallel, onProgress, zipPrepareLimiter = null) {
  ...
  async function runUnifiedImport(rawArgs, runOptions = {}) {
    ...
    const runSource = async (sourceEntry, sourceIndex) => {
      ...
      if (sourceEntry.kind === 'zip') {
        return importZipSource(sourceEntry.source, parsed, importParallel, onProgress, zipPrepareLimiter);
      }
