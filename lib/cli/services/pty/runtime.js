...
function createPtyRuntime(options = {}) {
  ...
  function runCliPty(cliName, initialId, forwardArgs, isLogin = false) {
    ...
    const idlePetFamily = idlePetFamilies[Math.floor(Math.random() * idlePetFamilies.length)];
    ...
    function formatIdleStatusLine() {
      ...
      const petFrames = idlePetFamily && Array.isArray(idlePetFamily.sleepFrames) && idlePetFamily.sleepFrames.length > 0
        ? idlePetFamily.sleepFrames
        : ['🐾'];
      const pet = petFrames[idleStatusTick % petFrames.length];
      return `\x1b[90m[aih]\x1b[0m sleeping${wave} ${zzz} ${pet}`;
    }

    function formatPlayStatusLine() {
      const wave = idleWaveFrames[idleStatusTick % idleWaveFrames.length];
      const petFrames = idlePetFamily && Array.isArray(idlePetFamily.playFrames) && idlePetFamily.playFrames.length > 0
        ? idlePetFamily.playFrames
        : ['🐾'];
      const pet = petFrames[idleStatusTick % petFrames.length];
      const toy = idlePetFamily && idlePetFamily.toy ? idlePetFamily.toy : '🎾';
      return `\x1b[90m[aih]\x1b[0m ${lastKnownUsageStatusSummary} | playing${wave} ${pet} ${toy}`;
    }
