...
function createPtyRuntime(options = {}) {
  ...
  function runCliPty(cliName, initialId, forwardArgs, isLogin = false) {
    ...
    function isClipboardPasteTrigger(data) {
      ...
      const shiftInsertPattern = /^\x1b\[2(?:;\d+)?~$/;
      const ctrlVCsiUPattern = /^\x1b\[(?:86|118);5(?:[:;]\d+)*u$/;
      const ctrlVModifyOtherKeysPattern = /^\x1b\[27;5;(?:86|118)(?:;\d+)*~$/;
      const ctrlVFunctionPattern = /^\x1b\[(?:1;)?5V$/;
      ...
      if (Buffer.isBuffer(data)) {
        ...
        if (shiftInsertPattern.test(text)) return true;
        if (ctrlVCsiUPattern.test(text)) return true;
        if (ctrlVModifyOtherKeysPattern.test(text)) return true;
        if (ctrlVFunctionPattern.test(text)) return true;
      ...
      if (text.length === 1 && text.charCodeAt(0) === 0x16) return true;
      if (shiftInsertPattern.test(text)) return true;
      if (ctrlVCsiUPattern.test(text)) return true;
      if (ctrlVModifyOtherKeysPattern.test(text)) return true;
      if (ctrlVFunctionPattern.test(text)) return true;
    ...
    function isEmptyBracketedPasteEnvelope(data) {
      if (!canBridgeWindowsClipboard()) return false;
    ...
    const onStdinData = (data) => {
      markSessionActivity();
      if (isClipboardPasteTrigger(data) || isEmptyBracketedPasteEnvelope(data)) {
    ...
    let lastUsageStatusPrintedAt = 0;
    let idleStatusTick = 0;
    let lastKnownUsageStatusSummary = `account ${activeId} usage remaining: unknown`;
    const idleWaveFrames = ['.', '..', '...'];
    const idleZFrames = ['zzz', 'zZz', 'Zzz', 'zzZ', 'zZZ'];
    const idleWorkerFamilies = [
      {
        label: 'cow-worker',
        playFrames: ['🐂💻', '🐂☕', '🐂📊'],
        sleepFrames: ['🐂😪', '🐂😴'],
    ...
    const idleWorkerFamily = idleWorkerFamilies[Math.floor(Math.random() * idleWorkerFamilies.length)];
    ...
    function startClipboardImageMirrorProcess() {
      if (!canBridgeWindowsClipboard()) return;
      if (String(processObj.env.AIH_WINDOWS_IMAGE_PASTE || '1') === '0') return;
      // Disabled by default: always-on clipboard polling can disturb foreground focus on Windows.
      // Image conversion should primarily happen on explicit paste keys from this active TTY.
      if (String(processObj.env.AIH_WINDOWS_IMAGE_CLIPBOARD_MIRROR || '0') !== '1') return;
      if (isLogin) return;
      ...
      const encoded = Buffer.from(psScript, 'utf16le').toString('base64');
      const candidates = wslMode ? ['powershell.exe', 'powershell'] : ['powershell', 'pwsh'];
    ...
    function formatIdleStatusLine() {
      const wave = idleWaveFrames[idleStatusTick % idleWaveFrames.length];
      const zzz = idleZFrames[idleStatusTick % idleZFrames.length];
      const workerFrames = idleWorkerFamily && Array.isArray(idleWorkerFamily.sleepFrames) && idleWorkerFamily.sleepFrames.length > 0
        ? idleWorkerFamily.sleepFrames
        : ['😪'];
      const worker = workerFrames[idleStatusTick % workerFrames.length];
      return `\x1b[90m[aih]\x1b[0m off-shift${wave} ${zzz} ${worker} (power-nap)`;
    }

    function formatPlayStatusLine() {
      const wave = idleWaveFrames[idleStatusTick % idleWaveFrames.length];
      const workerFrames = idleWorkerFamily && Array.isArray(idleWorkerFamily.playFrames) && idleWorkerFamily.playFrames.length > 0
        ? idleWorkerFamily.playFrames
        : ['🧑‍💻'];
      const worker = workerFrames[idleStatusTick % workerFrames.length];
      const tool = idleWorkerFamily && idleWorkerFamily.tool ? idleWorkerFamily.tool : '☕';
      return `\x1b[90m[aih]\x1b[0m ${lastKnownUsageStatusSummary} | working${wave} ${worker} ${tool}`;
    }
    ...
    function startUsageIdleStatusWatcher() {
      ...
      usageIdleStatusTimer = setInterval(() => {
        ...
        if (Date.now() - lastUsageStatusPrintedAt < getUsageStatusHoldMs()) return;
        idleStatusTick += 1;
    ...
    function emitUsageStatus(id, options = {}) {
      ...
      lastUsageStatusPrintedAt = Date.now();
    }
