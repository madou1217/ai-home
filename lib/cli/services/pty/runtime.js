...
function createPtyRuntime(options = {}) {
  ...
  function runCliPty(cliName, initialId, forwardArgs, isLogin = false) {
    ...
    const debugPtyStdin = String(processObj.env.AIH_DEBUG_PTY_STDIN || '0') === '1';
    if (canUseRawMode) {
    ...
    const onStdinData = (data) => {
      if (debugPtyStdin) {
        ...
        processObj.stdout.write(`\r\n\x1b[2m[aih][stdin] len=${chunk.length} hex=${hex} text="${text}"\x1b[0m\r\n`);
      }
    ...
    function startClipboardImageMirrorProcess() {
      ...
      const psScript = [
        ...
        'public static class AihNativeClipboard {',
        ...
        'while ($true) {',
        '  try {',
        '    $seq = [AihNativeClipboard]::GetClipboardSequenceNumber()',
