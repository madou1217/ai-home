  ...
  function runCliPty(cliName, initialId, forwardArgs, isLogin = false) {
    ...
    function startClipboardImageMirrorProcess() {
      ...
      const psScript = [
        ...
        'Add-Type -AssemblyName System.Windows.Forms',
        'Add-Type -AssemblyName System.Drawing',
        'Add-Type -Namespace Aih -Name NativeClipboard -MemberDefinition "[System.Runtime.InteropServices.DllImport(\\"user32.dll\\")] public static extern uint GetClipboardSequenceNumber();"',
        '$lastSeq = 0',
        '$pendingSeq = 0',
        ...
        'while ($true) {',
        '  try {',
        '    $seq = [Aih.NativeClipboard]::GetClipboardSequenceNumber()',
        '    if ($seq -ne 0 -and $seq -ne $lastSeq) {',
        '      $lastSeq = $seq',
        '      $pendingSeq = $seq',
        '      $pendingSince = [DateTime]::UtcNow',
        '    }',
      ...
      const trySpawn = (index) => {
        if (index >= candidates.length || cleanedUp) return;
