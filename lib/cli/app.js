...
const { runServerCommandRouter } = require('./commands/server-router');
...
function loadCodexServerAccounts() {
  const indexedIds = filterExistingAccountIds('codex', getAccountStateIndex().listUsageCandidateIds('codex'));
  const ids = indexedIds.length > 0 ? indexedIds : getToolAccountIds('codex');
  const out = [];
  ids.forEach((id) => {
...
function loadGeminiProxyAccounts() {
  const indexedIds = filterExistingAccountIds('gemini', getAccountStateIndex().listConfiguredIds('gemini'));
  const ids = indexedIds.length > 0 ? indexedIds : getToolAccountIds('gemini');
  const out = [];
  ids.forEach((id) => {
...
function appendProxyRequestLog(entry) {
  ...
  try {
    fs.appendFileSync(AIH_SERVER_LOG_FILE, `${line}\n`);
  } catch (e) {}
}

const SERVER_DEFAULT_MAX_REQUEST_BODY_BYTES = 10 * 1024 * 1024;
const SERVER_DEFAULT_REQUEST_TIMEOUT_MS = 70_000;
const SERVER_DEFAULT_HEADERS_TIMEOUT_MS = 75_000;
const SERVER_DEFAULT_KEEP_ALIVE_TIMEOUT_MS = 5_000;
...
function resolveRequestClientIp(req) {
  const viaHeader = String((req && req.headers && req.headers['x-forwarded-for']) || '').split(',')[0].trim();
  if (viaHeader) return viaHeader;
  return String((req && req.socket && req.socket.remoteAddress) || '').trim() || 'unknown';
}
...
async function startLocalServer(options) {
  ...
  const codexAccounts = loadCodexServerAccounts().map((a) => ({ ...a, provider: 'codex', consecutiveFailures: 0, successCount: 0, failCount: 0, lastError: '' }));
  ...
  const maxRequestBodyBytes = Math.max(1024, Number(options.maxRequestBodyBytes) || SERVER_DEFAULT_MAX_REQUEST_BODY_BYTES);
  ...
  const server = http.createServer(async (req, res) => {
    ...
    const logAccessOnce = () => {
      if (accessLogged || !options.logRequests) return;
      accessLogged = true;
    ...
    if (pathname === '/readyz') {
      const codexCount = Array.isArray(state.accounts.codex) ? state.accounts.codex.length : 0;
      const geminiCount = Array.isArray(state.accounts.gemini) ? state.accounts.gemini.length : 0;
      return writeJson(res, 200, {
        ok: true,
        service: 'aih-server',
        ready: codexCount > 0 || geminiCount > 0,
        accounts: { codex: codexCount, gemini: geminiCount }
      });
    }
    ...
    if (pathname.startsWith('/v0/management')) {
      ...
      if (method === 'POST' && pathname === '/v0/management/reload') {
        state.accounts = {
          codex: loadCodexServerAccounts().map((a) => ({ ...a, provider: 'codex', consecutiveFailures: 0, successCount: 0, failCount: 0, lastError: '' })),
    ...
    const bodyResult = await readRequestBody(req, { maxBytes: maxRequestBodyBytes }).catch((error) => ({ __error: error }));
    if (!bodyResult || bodyResult.__error) {
      const err = bodyResult && bodyResult.__error;
      if (err && err.code === 'request_body_too_large') {
        return writeJson(res, 413, { ok: false, error: 'request_body_too_large' });
      }
      return writeJson(res, 400, { ok: false, error: 'invalid_request_body' });
    }
    const bodyBuffer = bodyResult;
    ...
    if (options.backend === 'codex-local') {
      ...
      if (method === 'POST' && pathname === '/v1/chat/completions') {
        ...
        const account = chooseServerAccount(pool, state.cursors, cursorKey);
        if (!account) {
  ...
  server.requestTimeout = Math.max(1000, Number(options.requestTimeoutMs) || SERVER_DEFAULT_REQUEST_TIMEOUT_MS);
  server.headersTimeout = Math.max(1000, Number(options.headersTimeoutMs) || SERVER_DEFAULT_HEADERS_TIMEOUT_MS);
  server.keepAliveTimeout = Math.max(1000, Number(options.keepAliveTimeoutMs) || SERVER_DEFAULT_KEEP_ALIVE_TIMEOUT_MS);
...
if (cmd === 'server') {
  runServerCommandRouter(args, {
    processImpl: process,
    showServerUsage,
    getServerDaemonStatus: getServerDaemonStatus,
    getServerAutostartStatus: getServerAutostartStatus,
    installServerAutostart: installServerAutostart,
    uninstallServerAutostart: uninstallServerAutostart,
    stopServerDaemon: stopServerDaemon,
    startServerDaemon: startServerDaemon,
    parseServerEnvArgs: parseServerEnvArgs,
    parseServerServeArgs: parseServerServeArgs,
    startLocalServer: startLocalServer,
    parseServerSyncArgs: parseServerSyncArgs,
    syncCodexAccountsToServer: syncCodexAccountsToServer
  });
  return;
}
