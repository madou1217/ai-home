...
const { runProxyCommandRouter } = require('./commands/proxy-router');
...
\x1b[33mUsage:\x1b[0m
  ...
  aih codex account import [dir] [--parallel N] [--limit N] [--dry-run]\x1b[90mBulk import codex refresh tokens from output files\x1b[0m
...
\x1b[33mUsage:\x1b[0m
  ...
  ${cliName === 'codex' ? 'aih codex account import [dir] [--parallel N] [--limit N] [--dry-run]  \x1b[90mBulk import codex refresh tokens\x1b[0m' : ''}
...
async function startProxyDaemon(rawServeArgs) {
  ...
  const child = spawn(process.execPath, [__filename, 'server', 'serve', ...(rawServeArgs || [])], {
    detached: true,
...
if (cmd === 'server') {
  runProxyCommandRouter(args, {
    processImpl: process,
    showProxyUsage: showServerUsage,
    getProxyDaemonStatus,
    getProxyAutostartStatus,
