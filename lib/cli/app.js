...
if (idOrAction && ACCOUNT_ACTIONS.has(idOrAction)) {
  if (cliName !== 'codex') {
    console.error(`\x1b[31m[aih] ${cliName} account subcommand is currently supported only for codex.\x1b[0m`);
    process.exit(1);
  }
  const accountAction = String(args[2] || '').trim().toLowerCase();
  if (accountAction !== 'import') {
    console.error(`\x1b[31m[aih] Unknown account action '${accountAction || ''}'.\x1b[0m`);
