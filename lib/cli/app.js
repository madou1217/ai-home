...
const AIH_SERVER_MANAGEMENT_BASE = String(process.env.AIH_SERVER_MANAGEMENT_URL || 'http://127.0.0.1:8317/v0/management').trim().replace(/\/+$/, '');

function postStateIndexMutation(route, payload) {
  if (!route || typeof fetch !== 'function') return;
  // Single-writer architecture: CLI never writes SQLite directly.
  // It delegates state-index mutations to the running server.
  const url = `${AIH_SERVER_MANAGEMENT_BASE}${route}`;
  const headers = {
    'content-type': 'application/json'
  };
  const managementKey = String(process.env.AIH_SERVER_MANAGEMENT_KEY || '').trim();
...
function upsertAccountStateViaServer(provider, accountId, state) {
  postStateIndexMutation('/state-index/upsert', {
...
function setExhaustedViaServer(provider, accountId, exhausted) {
  postStateIndexMutation('/state-index/set-exhausted', {
    provider: String(provider || '').trim(),
    accountId: String(accountId || '').trim(),
    exhausted: !!exhausted
  });
...
function pruneMissingViaServer(provider, existingIds) {
  postStateIndexMutation('/state-index/prune-missing', {
    provider: String(provider || '').trim(),
...
function buildMockUsageSnapshot(cliName, remainingPct) {
  ...
  if (cliName === 'codex' || cliName === 'claude') {
    return {
      ...
      source: cliName === 'codex' ? USAGE_SOURCE_CODEX : USAGE_SOURCE_CLAUDE_OAUTH,
      capturedAt: now,
      entries: [
        {
          bucket: 'primary',
...
function startCodexAppServer() {
  if (process.platform === 'win32') {
    // On Windows, codex is commonly installed as a .cmd shim.
    // Spawning through cmd.exe avoids silent launch failures in usage probes.
...
function clearExhausted(cliName, id) {
  const p = path.join(getProfileDir(cliName, id), '.aih_exhausted');
  if (!fs.existsSync(p)) {
    setExhaustedViaServer(cliName, id, false);
    return false;
  }
  try {
    fs.unlinkSync(p);
...
function markExhaustedFromUsage(cliName, id) {
  ...
  try {
    ...
    setExhaustedViaServer(cliName, id, true);
    return true;
  } catch (e) {
    return false;
  }
...
function syncExhaustedStateFromUsage(cliName, id) {
  ...
  if (isUsageSnapshotExhausted(cache)) {
    ...
    upsertAccountStateViaServer(cliName, id, {
      configured: true,
      apiKeyMode: false,
      exhausted: true,
...
function refreshIndexedStateForAccount(cliName, id, options = {}) {
  ...
  upsertAccountStateViaServer(cliName, accountId, {
    configured,
    apiKeyMode,
    exhausted,
    remainingPct,
...
function getToolAccountIds(cliName) {
  ...
  pruneMissingViaServer(cliName, fsIds);
  return fsIds;
}
...
function getNextAvailableId(cliName, currentId) {
  ...
  for (let i = 0; i < 128; i += 1) {
    ...
    if (!fs.existsSync(profileDir)) {
      pruneMissingViaServer(cliName, listToolProfileIdsFromFs(cliName));
      continue;
    }
    const { configured, accountName } = checkStatus(cliName, profileDir);
    const apiKeyMode = !!(accountName && accountName.startsWith('API Key'));
    ...
    upsertAccountStateViaServer(cliName, indexedCandidate, {
      configured,
      apiKeyMode,
      exhausted
    });
...
function spawnPty(cliName, cliBin, id, forwardArgs, isLogin) {
  ...
  const argsToRunBase = isLogin ? (CLI_CONFIGS[cliName]?.loginArgs || []) : forwardArgs;
  const argsToRun = Array.isArray(argsToRunBase) ? [...argsToRunBase] : [];
  if (
    cliName === 'codex'
    && !isLogin
    && String(process.env.AIH_CODEX_AUTO_SKIP_REPO_CHECK || '1') !== '0'
...
function runCliPty(cliName, initialId, forwardArgs, isLogin = false) {
  ...
  function getThresholdPct() {
    const cfg = readUsageConfig({ filePath: path.join(AI_HOME_DIR, 'usage-config.json') });
  ...
  function startThresholdWatcher() {
    const enabled = String(process.env.AIH_RUNTIME_AUTO_SWITCH || '1') !== '0';
    const interactive = !isLogin && (!Array.isArray(forwardArgs) || forwardArgs.length === 0);
    if (!enabled || !interactive || cliName !== 'codex') return;
    const intervalMs = Math.max(30_000, Number(process.env.AIH_RUNTIME_THRESHOLD_CHECK_MS) || 60_000);
...
if (cmd === 'dev') {
  ...
  if (action !== 'mock-usage') {
    console.error('\x1b[31m[aih] Unknown dev action.\x1b[0m');
    console.log('\x1b[90mUsage:\x1b[0m aih dev mock-usage <provider> <id> [--remaining <pct>] [--duration-sec <sec>]');
    process.exit(1);
  }

  const provider = String(args[2] || '').trim().toLowerCase();
  const id = String(args[3] || '').trim();
  if (!['codex', 'gemini', 'claude'].includes(provider) || !/^\d+$/.test(id)) {
    console.error('\x1b[31m[aih] Invalid provider or account id.\x1b[0m');
    console.log('\x1b[90mUsage:\x1b[0m aih dev mock-usage <provider> <id> [--remaining <pct>] [--duration-sec <sec>]');
    process.exit(1);
  }
  ...
  const mockSnapshot = buildMockUsageSnapshot(provider, remainingPct);

  console.log(`\x1b[36m[aih]\x1b[0m Mock usage start: ${provider}#${id}, remaining=${remainingPct.toFixed(1)}%, duration=${durationSec}s`);
  fs.writeFileSync(usagePath, `${JSON.stringify(mockSnapshot, null, 2)}\n`);
  const mocked = readUsageCache(provider, id);
  if (!mocked) {
    console.error('\x1b[31m[aih] mock write failed: snapshot not readable after write.\x1b[0m');
    process.exit(1);
  }
  console.log(`\x1b[32m[aih]\x1b[0m Mock injected at ${usagePath}`);
  ...
  setTimeout(() => {
    try {
      ...
      if (hadOriginal && !restored) {
        console.error('\x1b[31m[aih] restore check failed: original snapshot is no longer readable.\x1b[0m');
        process.exit(1);
      }
      console.log('\x1b[32m[aih]\x1b[0m Mock usage restored and re-checked.');
      process.exit(0);
    } catch (error) {
      console.error(`\x1b[31m[aih] restore failed: ${error.message}\x1b[0m`);
      process.exit(1);
    }
  }, durationSec * 1000);
  return;
