...
\x1b[33mUsage:\x1b[0m
  ...
  aih <cli> account import [dir] [--parallel N] [--limit N] [--dry-run]\x1b[90mImport account tokens/state when supported by provider\x1b[0m
...
\x1b[33mUsage:\x1b[0m
  ...
  aih ${cliName} account import [dir] [--parallel N] [--limit N] [--dry-run]  \x1b[90mImport account tokens/state when supported\x1b[0m
...
\x1b[33mAdvanced (optional):\x1b[0m
  aih server serve [--host <ip>] [--port <n>] [--backend codex-local|openai-upstream] [--provider codex|gemini|auto] [--upstream <url>] [--strategy round-robin|random] [--client-key <key>] [--management-key <key>] [--cooldown-ms <ms>] [--max-attempts <n>] [--upstream-timeout-ms <ms>]
...
const serverDaemonService = createServerDaemonService({
  ...
  processObj: process,
  ensureDir,
  parseServeArgs: parseProxyServeArgs,
...
function readProxyPid() {
  return serverDaemonService.readPid();
}

function isProcessAlive(pid) {
  return serverDaemonService.isAlive(pid);
}

function waitForProxyReady(port, timeoutMs = 5000) {
  return serverDaemonService.waitForReady(port, timeoutMs);
}
...
if (cmd === 'server') {
  runServerCommandRouter(args, {
    processImpl: process,
    showServerUsage,
    getServerDaemonStatus: getProxyDaemonStatus,
    getServerAutostartStatus: getProxyAutostartStatus,
    installServerAutostart: installProxyAutostart,
    uninstallServerAutostart: uninstallProxyAutostart,
    stopServerDaemon: stopProxyDaemon,
    startServerDaemon: startProxyDaemon,
    parseServerEnvArgs: parseProxyEnvArgs,
    parseServerServeArgs: parseProxyServeArgs,
    startLocalServer: startLocalProxyServer,
    parseServerSyncArgs: parseProxySyncArgs,
    syncCodexAccountsToServer: syncCodexAccountsToProxy
  });
  return;
}
