...
function getToolAccountIds(cliName) {
  const toolDir = path.join(PROFILES_DIR, cliName);
  const fsIds = fs.existsSync(toolDir)
    ? fs.readdirSync(toolDir)
    .filter((f) => /^\d+$/.test(f) && fs.statSync(path.join(toolDir, f)).isDirectory())
    : [];
  const indexedIds = getAccountStateIndex().listAccountIds(cliName);
  if (indexedIds.length === 0) {
    return fsIds.sort((a, b) => parseInt(a, 10) - parseInt(b, 10));
  }
  const merged = new Set([...indexedIds, ...fsIds]);
  return Array.from(merged).sort((a, b) => parseInt(a, 10) - parseInt(b, 10));
}
...
async function printAllUsageSnapshots(cliName) {
  const indexedOauthIds = getAccountStateIndex().listUsageCandidateIds(cliName);
      ...
      while (cursor < ids.length) {
        ...
        batch.forEach(id => {
          ...
          if (useFastIndexView && indexedMap.has(id)) {
            ...
            accountName = row.apiKeyMode ? 'API Key' : (configured ? 'Indexed' : 'Unknown');
            exhaustedFlag = !!row.exhausted;
            if (typeof row.remainingPct === 'number') {
...
function loadCodexServerAccounts() {
  const indexedIds = getAccountStateIndex().listUsageCandidateIds('codex');
...
function loadGeminiProxyAccounts() {
  const indexedIds = getAccountStateIndex().listConfiguredIds('gemini');
