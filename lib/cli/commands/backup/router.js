...
function stageSelectedProfilesAsAccounts({ fs, path, fse, aiHomeDir, targetPaths, stageRoot, onProgress = null }) {
  ...
  selectedAccounts.forEach((account, index) => {
    ...
    let hiddenEntries = [];
    try {
      rootEntries = fs.readdirSync(account.profileDir, { withFileTypes: true });
    } catch (_error) {}
    try {
      hiddenEntries = fs.readdirSync(path.join(account.profileDir, providerHiddenDir), { withFileTypes: true });
    } catch (_error) {}
    ...
    if (current === selectedAccounts.length || current % 100 === 0) {
      emitProgress(current, selectedAccounts.length, {
