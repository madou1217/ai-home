...
function stageSelectedProfilesAsAccounts({ fs, path, fse, aiHomeDir, targetPaths, stageRoot }) {
  ...
  if (seen.has('profiles')) {
    if (!fs.existsSync(profilesRoot) || !fs.statSync(profilesRoot).isDirectory()) {
      return { accountsDir, providerDirs: [] };
    }
    const entries = fs.readdirSync(profilesRoot, { withFileTypes: true });
    entries.forEach((entry) => {
      const src = path.join(profilesRoot, entry.name);
      const dst = path.join(accountsDir, entry.name);
      fse.copySync(src, dst, { overwrite: true });
    });
    return {
      accountsDir,
      providerDirs: listChildDirectories(fs, path, accountsDir)
    };
  }
  ...
  Array.from(seen).forEach((raw) => {
    ...
    const suffix = rel.slice('profiles/'.length).trim();
    if (!suffix) return;

    const src = path.join(aiHomeDir, rel);
    if (!fs.existsSync(src)) return;
    const dst = path.join(accountsDir, suffix);
    fse.copySync(src, dst, { overwrite: true });
  });
...
async function runBackupCommand(cmd, args, deps = {}) {
  ...
  if (cmd === 'export') {
    ...
    if (selectors.length > 0) {
      ...
      consoleImpl.log('\x1b[36m[aih]\x1b[0m Exporting all profiles into account-import compatible layout (accounts/<provider>/...).');
    }
    ...
    try {
      ...
      if (staged.providerDirs.length === 0) {
        throw new Error('No account directories found to export.');
      }
