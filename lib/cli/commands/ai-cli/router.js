...
function runAiCliCommandRouter(cmd, args, context = {}) {
  ...
  const refreshAccountStateIndexForProvider = context.refreshAccountStateIndexForProvider;
  ...
  const KNOWN_ACTIONS = new Set(['ls', 'set-default', 'login', 'auto', ...POLICY_ACTIONS, ...ACCOUNT_ACTIONS, ...UNLOCK_ACTIONS, ...USAGE_ACTIONS]);
  ...
  if (idOrAction && ACCOUNT_ACTIONS.has(idOrAction)) {
    ...
    if (accountAction === 'import' && typeof runUnifiedImport === 'function') {
      Promise.resolve(runUnifiedImport(args.slice(3), {
        provider: cliName,
        log: console.log,
        error: console.error,
        renderStageProgress
        ...
        .then((result) => {
          result.providers.forEach((provider) => {
            if (typeof refreshAccountStateIndexForProvider === 'function') {
