...
async function runRootAccountCommand(args, deps = {}) {
  ...
  try {
    const result = await deps.runUnifiedImport(args.slice(2), {
      log: consoleImpl.log,
      error: consoleImpl.error,
      renderStageProgress: deps.renderStageProgress
    });
    if (result.failedSources.length > 0) {
      consoleImpl.error(`\x1b[31m[aih] account import completed with failures: ${result.failedSources.map((item) => item.source).join(', ')}\x1b[0m`);
      processObj.exit(1);
      return;
    }
